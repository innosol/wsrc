...
//ehlel
    ...
    jQuery(".gold .gold-footer button[data-data='gold']").click(function(e){
        jQuery('.overlay').hide();
        jQuery(this).parents(".gold").hide();
    });
...
function table_add_rm(tbody_id, add_button_id) {
    ...
    $(add_button_id).click(function() {
        ...
        $('input', 'tr:last', tbody_id).each(function(td_each){
            $(this).val('');
        });
        $('tr', tbody_id).each(function(j){
        $('td:first', this).text(j+1);
    });
    $("tr:last td:last", tbody_id).append("<a href='#' id='rm-row'>Устгах</a>");
        i++;
        return false;
    });
...
var formAjaxSubmit = function(form, modal) {
    $(form).submit(function (e) {
        ...
        $(this).children().children('button').prop('disabled', true);
        $.ajax({
            ...
            success: function (xhr, ajaxOptions, thrownError) {
                if ( $(xhr).find('.errorlist').length > 0 ) {
                    ...
                    $(form).children().children('button').prop('disabled', false);
                    ...
                    location.reload();
                    alert('Таны мэдээлэл амжиттай хадгалагдлаа')
                }
            },
...
var formModalAjaxSubmit = function(form, modal) {
    $(form).submit(function (e) {
        e.preventDefault();
        var data = new FormData($(this).get(0));
        $(this).children().children('button').prop('disabled', true);
...
formAjaxSubmit('#employee form', '#employee');
//formModalAjaxSubmit('#Modal_general form', '#Modal_general');

var insertAjaxSubmit = function(form, modal) {
    $(form).submit(function (e) {
        e.preventDefault();
        var data = new FormData($(this).get(0));
        $.ajax({
            ...
            success: function (xhr, ajaxOptions, thrownError) {
                if ( $(xhr).find('.errorlist').length > 0 ) {
                    var hh = $(xhr).find(form).parent('div').find('.modal1-uuganaa-body');
                    $(modal).find('.modal1-uuganaa-body').html(hh);
                }
                else {
                    location.reload();
                }
...
$('button[type = submit]').click(function(){
    ...
    /*$($(this).parent().parent()).on('submit', function(event) {
        ...
        var button = $(this).children('.modal1-uuganaa-footer').children('button')
...
$(document).ready(function(){
    if(".alert.alert-block.alert-success.fade.in"){
        setTimeout( function(){
            $(".alert.alert-block.alert-success.fade.in").hide();
        }, 1000*10);
    }
});